  ...
  if('serviceWorker' in navigator)
  {
    window.addEventListener('load', async () => {
      try{
        ...
        console.log("Successful log: ", registration)
      }
      catch(error)
      {
...
async function getRecipes() {
  ...
  return new Promise(async (resolve, reject) => {
    ...
    for (let i = 0; i < RECIPE_URLS.length; i++) {
      // A5. Use try/catch block for asynchronous fetch logic
      try {
        // A6. Fetch the URL
        ...
        // A9. If all recipes have been fetched, save to storage and resolve
